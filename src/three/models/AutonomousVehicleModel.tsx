...
export default function AutonomousVehicleModel({ onActiveIndexChange, paused }: TechModelProps) {
  ...
  const sensorColor = activeIndex === 0 ? '#A72B2B' : '#ACA8A7'
  const laneColor = activeIndex === 1 ? '#A72B2B' : '#736662'
  const pathColor = activeIndex === 2 ? '#A72B2B' : '#736662'

  return (
    <group position={[0, 0.55, 0]}>
      <group ref={bodyRef}>
        <WireBox size={[1.7, 0.4, 0.95]} color="#736662" position={[0, 0, 0]} />
        <WireBox size={[0.9, 0.35, 0.8]} color="#736662" position={[-0.1, 0.35, 0]} />
        ...
        <PulseRing active={activeIndex === 3} position={[0, -0.2, 0]} radius={1.2} color="#A72B2B" flat={false} />
      </group>
